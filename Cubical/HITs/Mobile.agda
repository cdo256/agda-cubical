...
module Mobile where
  data Mobile : Type where
    leaf : Mobile
    node : (ℕ → Mobile) → Mobile
    perm : (f : ℕ → Mobile) (π : ℕ → ℕ) (isIso-π : isIso π)
         → node f ≡ node (f ∘ π)
    isSetMobile : isSet Mobile 

  record Algebra : Type₁ where
    field
      Xᴬ : Type
      leafᴬ : Xᴬ
      nodeᴬ : (ℕ → Xᴬ) → Xᴬ
      permᴬ : (f : ℕ → Xᴬ) (π : ℕ → ℕ) (isIso-π : isIso π)
            → nodeᴬ f ≡ nodeᴬ (f ∘ π)
      isSetXᴬ : isSet Xᴬ
    
  record Hom (A B : Algebra) : Type₁ where
    module A = Algebra A
    module B = Algebra B
    field
      fᴿ : A.Xᴬ → B.Xᴬ
      leafᴿ : fᴿ A.leafᴬ ≡ B.leafᴬ
      nodeᴿ : ∀ (f : ℕ → Xᴬ) → fᴿ A.leafᴬ ≡ B.leafᴬ
...
--   -- iterTree : (M : Tree → Type) (l : (A → M (leaf A))) (n : (B → ∀ i → M (f i)))
--   iterTree : {M : Type} (l : A → M) (n : (B → M) → M) → Tree → M
--   iterTree l n (leaf x) = l x
--   iterTree l n (node f) = n (λ i → iterTree l n (f i))

--   recTree : {M : Tree → Type}
--           → (l : ∀ x → M (leaf x))
--           → (n : ∀ f → (g : ∀ i → M (f i)) → M (node f))
--           → ∀ t → M t
--   recTree l n (leaf x) = l x
--   recTree {M} l n (node f) = n f (λ i → recTree {M = M} l n (f i))

--   node≢leaf : ∀ {f x} → node f ≢ leaf x
--   node≢leaf n≡l = subst C n≡l tt 
--     where
--     C : Tree → Type
--     C (leaf x) = ⊥
--     C (node x) = Unit

--   isNode : ∀ (t : Tree) → hProp ℓ-zero
--   isNode t = ∃[ f ∶ (B → Tree) ] t ≡ₚ node f

--   isInjective-node : ∀ (f g : B → Tree) → node f ≡ node g → f ≡ g
--   isInjective-node f g p = {!!}
--     where
--       isNode-pi : (i : I) → ⟨ isNode (p i) ⟩
--       isNode-pi i = {!Σ≡PathP!}
--       -- A = Σ[ t ∈ Tree ] isNode t
--       -- P : ∀ (a : A) → (a ≡ b) → Type
--       -- Prefl : P nf
